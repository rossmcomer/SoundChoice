...
<template>
    ...
    <div class="relative h-full z-10">
      ...
      <div class="p-4 space-y-4">
        <button @click="adminStore.fetchClientEmails" class="bg-blue-500 text-white px-4 py-2 rounded cursor-pointer">
  Load Client Emails
</button>
    ...
    <button @click="adminStore.fetchBookings" class="bg-green-500 text-white px-4 py-2 rounded cursor-pointer">
  Load All Bookings
</button>
  ...
  >
    <div class="text-black font-medium">
      {{ booking.user.name }} – {{ new Date(booking.eventDate).toLocaleDateString() }} – {{ booking.location }}
    </div>
    <button
